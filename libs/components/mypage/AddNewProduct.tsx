...
/** UI helper: ENUM labelini chiroyli chiqarish (WHITE_GOLD -> White Gold) */
...
const AddProduct: React.FC<AddProductProps> = ({ initialValues }) => {
  ...
  /** Enum ro‘yxatlari */
  const [productTypeList] = useState<ProductType[]>(
    Object.values(ProductType)
  );
  const [productLocationList] = useState<ProductLocation[]>(
    Object.values(ProductLocation)
  );
  const [productMaterialList] = useState<ProductMaterial[]>(
  ...
  async function uploadImages() {
    try {
      ...
      const response = await axios.post(
        // Istasangiz NEXT_PUBLIC_API_GRAPHQL_URL ga o‘tkazing
      ...
      const responseImages: string[] =
        response.data?.data?.imagesUploader ?? [];
        ...
        <Stack className="config">
          <Stack className="description-box">
            ...
            <Stack className="config-row">
              ...
              {/* Qo'shimcha joy bo'sh qoldirildi (kerak bo'lsa yana maydon qo'shasiz) */}
              <Stack className="price-year-after-price" />
            </Stack>
